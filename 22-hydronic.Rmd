...
Le **régime de dimensionnement** d'un émetteur est déterminé par les températures d'entrée et de sortie de l'eau en conditions nominales (voir section \@ref(sec:dimensionnement)). La figure \@ref(fig:regime) montre l'exemple d'un radiateur de 2000 W dimensionné en régime 70/50 : il est alimenté avec de l’eau à 70°C, celle-ci cédera 2000 W de chaleur au local à 20°C, et ressortira avec une température de 50°C.

```{r, regime, echo=FALSE, out.width="40%", fig.cap="Radiateur en régime 70/50"}
...
```{r, stratification, echo=FALSE, out.width="40%", fig.cap="Exemple de stratification des températures avec un chauffage par radiateur et un chauffage par convecteur : la stratification est plus importante dans le deuxième cas, entraînant des pertes de chaleur à cause de la température élevée au plafond, bien que la température soit la même au niveau du thermostat."}
...
```{r, circuit, echo=FALSE, out.width="80%", fig.cap="Exemple de réseau avec une boucle primaire et plusieurs boucles secondaires. La circulation de l'eau est assurée par un circulateur sur chaque boucle qui permet d'en gérer les débits indépendamment. Une vanne 3 voies peut être placée au départ de chaque circuit secondaire."}
...
```{r, bouclefermee, echo=FALSE, out.width="80%", fig.cap="Deux circuits en boucle fermée, avec bouteille casse-pression (en bas) ou sans (en haut)."}
...
```{r, equilibrage, echo=FALSE, out.width="40%", fig.cap="Un défaut d'équilibrage (à gauche) peut entraîner une mauvaise répartition des températures d'arrivée de l'eau dans les radiateurs."}
...
```{r, vannes, echo=FALSE, out.width="40%", fig.cap="Robinet thermostatique, té de réglage, vanne d'équilibrage"}
...
```{r, circulateur0, echo=FALSE, out.width="40%", fig.cap="Circulateur à vitesse variable"}
...
```{r, circulateur2, echo=FALSE, out.width="40%", fig.cap="Détermination du point de fonctionnement en fonction des caractéristiques du réseau et du circulateur"}
...
```{r, chaudieres, echo=FALSE, out.width="40%", fig.cap="En haut : déperditions thermiques sur une chaudière. En bas : principe d'une chaudière à condensation."}
...
\begin{align}
    P_\mathrm{utile} & = \dot{m}_\mathrm{eau} \, c_p \, \left(T_\mathrm{depart} - T_\mathrm{retour} \right) \\
    P_\mathrm{conso} & = PCI \times \dot{m}_\mathrm{combustible}
\end{align}
...
```{r, pac, echo=FALSE, out.width="40%", fig.cap="Principe d'une pompe à chaleur, ici en mode chauffage"}
...
```{r, pacabsorption, echo=FALSE, out.width="40%", fig.cap="Principe d'une pompe à chaleur à absorption"}
...
\begin{align}
    COP_\mathrm{chaud} & \leq \frac{T_\mathit{chaud}}{T_\mathit{chaud}-T_\mathit{froid}} \\
    COP_\mathrm{froid} & \leq \frac{T_\mathit{froid}}{T_\mathit{chaud}-T_\mathit{froid}} \\
\end{align}
...
```{r, geothermie, echo=FALSE, out.width="70%", fig.cap="Trois types de PAC géothermiques"}
knitr::include_graphics(rep("figures/schema-explicatif-geothermie.jpg"))
```
