...
```{r, ecsstandard, echo=FALSE, out.width="40%", fig.cap="Coefficients d'équivalence permettant de calculer le nombre de logements standards. Par exemple, un appartement T4 dans un immeuble du parc social est considéré égal à 1,4 logement standard."}
...
Les **besoins moyens** sont déterminés en fonction de son nombre de logements standards (voir figure \@ref(fig:ecsstandard). Le logement standard est un T3 d'un immeuble du parc social, occupé par 2,1 personnes. Les besoins journaliers moyens d'un immeuble sur l'année sont, en moyenne, de 125 $\pm$ 50 litres à 40$\degree$C par logement standard, soit environ 70 $\pm$ 25 litres à 60$\degree$C, pour une température moyenne annuelle d'eau froide de 16$\degree$C.

On a aussi besoin de connaître les **besoins de pointes** sur 10 minutes, ou 1 à 8h. Ces besoins de pointes, en litres à 60$\degree$C, pour une température minimale moyenne d'eau froide à 9$\degree$C, peuvent être estimés avec le nombre de logements standards $N_s$ :
\begin{align}
...
```{r, profilecs, echo=FALSE, out.width="40%", fig.cap="Exemple de profils de soutirage moyens horaires, hebdomadaires et mensuels sur un échantillon de logements standards du parc social."}
...
\begin{equation}
    P_\mathrm{prod,inst} = \frac{\rho \, c_p}{1000} \frac{Vp_\mathrm{10min}}{600} \left(60-9\right) + P_\mathrm{distrib}
(\#eq:ecsinstantane)
\end{equation}
où $P_\mathrm{distrib}$ sont les pertes de distribution. Dans cette formule, le volume puisé en 10 minutes est divisé par 600 (s) pour le convertir en débit volumique en litres par secondes.
...
On détermine d'abord l'énergie $E_\mathrm{acc}$ nécessaire pour satisfaire à la demande maximale de l'année pendant une journée. Sur cette base, on dimensionne le préparateur à accumulation en déterminant sa puissance et son volume. L'énergie $E_\mathrm{acc}$ est obtenue à partir des profils journaliers maximaux. Par exemple si ces profils sont donnés par un ensemble de puisages (ici en litres) à différentes températures, avec une température d'eau froide de 9$\degree$C :
\begin{equation}
    E_\mathrm{acc} = \frac{\rho \, c_p}{1000} \sum_\mathrm{puisages} V_\mathrm{puisage} \left(T_\mathrm{puisage}-9\right)
\end{equation}
Où $E_\mathrm{acc}$ est donné ici en Joules (rappel : pour le convertir en kWh, on divise par $3,6\times 10^6$). Le volume de stockage (ici en m$^3$) se calcule ensuite en choisissant une température de stockage $T_\mathrm{acc}$ de 55 ou 60$\degree$C
\begin{equation}
...
```{r, profilmaxecs, echo=FALSE, out.width="80%", fig.cap="Exemple de profil journalier maximal au pas de temps 10 minutes dans un ensemble de 208 logements du parc social"}
...
```{r, cumuluselectrique, echo=FALSE, out.width="80%", fig.cap="Composition et dimensionnement d'un chauffe-eau électrique"}
...
```{r, chaudieredoubleservice, echo=FALSE, out.width="40%", fig.cap="Exemple d'une chaudière double service dotée d'un ballon d'ECS avec un échangeur à serpentin"}
...
```{r, cesi, echo=FALSE, out.width="40%", fig.cap="Chauffe-eau solaire individuel avec appoint par une chaudière instantanée"}
...
```{r, ssc, echo=FALSE, out.width="40%", fig.cap="Système solaire combiné"}
knitr::include_graphics(rep("figures/ssc.png"))
```

Comme pour un CESI, un appoint est nécessaire pour couvrir les besoins de chauffage et d'ECS toute l'année. Le dimensionnement des composants du SSC sera abordé en section \@ref(sec:solaire).
...
```{r, couverture, echo=FALSE, out.width="40%", fig.cap="Evolution de la production solaire au long de l'année, et évolution de la productivité solaire par mètre carré de capteurs"}
